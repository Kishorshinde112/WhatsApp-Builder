  ...
  return (
    <>
      ...
      <div className="mt-auto border-t p-4 flex flex-col gap-2">
        <Link href="/settings/providers" onClick={onNavClick}>
          <div
            className={`flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors cursor-pointer ${
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen w-full bg-background text-foreground">
      {/* Desktop sidebar */}
      <aside className="hidden md:fixed md:top-0 md:left-0 md:z-20 md:flex md:h-full md:w-64 md:flex-col border-r bg-sidebar">
      ...
      {/* Mobile header bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-30 flex h-14 items-center border-b bg-sidebar px-4 gap-3">
        <button
      ...
      {/* Mobile drawer */}
      <aside
        className={`md:hidden fixed top-0 left-0 z-50 flex h-full w-64 flex-col border-r bg-sidebar transform transition-transform duration-200 ${
      ...
      >
        ...
        <NavContent location={location} onNavClick={() => setMobileOpen(false)} />
      </aside>
